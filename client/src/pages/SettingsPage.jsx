...
export default function SettingsPage() {
  ...
  const discordBannerStyle = user?.discord_banner
    ...
    : user?.discord_accent_color
      ...
      : { background: 'linear-gradient(135deg, #1a1a2e 0%, #16213e 100%)', height: 60 }
  ...
  return (
    <div className="settings-page">
      ...
      <div className="settings-section">
        ...
        <div style={{ background: 'var(--bg-surface)', border: '1px solid var(--border)', borderRadius: 'var(--radius-lg)', overflow: 'hidden' }}>
          ...
          <div style={{ padding: '0 24px 20px', display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(180px, 1fr))', gap: 12 }}>
            {user?.discord_locale && (
              <div style={{ padding: '12px 14px', background: 'rgba(255,255,255,0.03)', borderRadius: 'var(--radius-md)', border: '1px solid var(--border)' }}>
                <div style={{ fontSize: 11, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 4 }}>Locale</div>
                <div style={{ fontSize: 13, color: 'var(--text)', display: 'flex', alignItems: 'center', gap: 6 }}><Globe size={14} /> {user.discord_locale}</div>
              </div>
            )}
            {user?.created_at && (
              <div style={{ padding: '12px 14px', background: 'rgba(255,255,255,0.03)', borderRadius: 'var(--radius-md)', border: '1px solid var(--border)' }}>
                <div style={{ fontSize: 11, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 4 }}>Joined</div>
                <div style={{ fontSize: 13, color: 'var(--text)' }}>{new Date(user.created_at).toLocaleDateString()}</div>
              </div>
            )}
            <div style={{ padding: '12px 14px', background: 'rgba(255,255,255,0.03)', borderRadius: 'var(--radius-md)', border: '1px solid var(--border)' }}>
      ...
      <div className="settings-section">
        ...
        {twoFASuccess && (
          <div style={{ padding: '12px 16px', borderRadius: 'var(--radius-md)', background: 'rgba(34,197,94,0.1)', color: 'var(--success)', fontSize: 13, marginBottom: 14, border: '1px solid rgba(52,211,153,0.2)' }}>
            {twoFASuccess}
          </div>
        )}
        <div style={{ background: 'var(--bg-surface)', backdropFilter: 'blur(20px)', border: '1px solid var(--border)', borderRadius: 'var(--radius-lg)', padding: 24, boxShadow: 'var(--shadow-md)' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
            <div>
              <div style={{ fontSize: 14, fontWeight: 600, color: 'var(--text)' }}>Two-Factor Authentication</div>
            ...
            <div style={{ marginTop: 16 }}>
              ...
              {twoFAState.qrCode && (
                <div style={{ marginBottom: 14, textAlign: 'center' }}>
                  <img src={twoFAState.qrCode} alt="2FA QR Code" style={{ width: 200, height: 200, borderRadius: 'var(--radius-md)', border: '1px solid var(--border)' }} />
                </div>
              )}
              <div style={{ marginBottom: 14 }}>
                <div style={{ fontSize: 12, color: 'var(--text-muted)', marginBottom: 6 }}>Or enter this secret manually:</div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '10px 14px', background: 'rgba(0,0,0,0.2)', borderRadius: 'var(--radius-md)', border: '1px solid var(--border)', fontFamily: 'var(--mono)', fontSize: 13, color: 'var(--text)', letterSpacing: 1 }}>
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                <input
                  ...
                  maxLength={6}
                  style={{ fontFamily: 'var(--mono)', fontSize: 18, letterSpacing: 4, textAlign: 'center', flex: 1 }}
                  autoFocus
                />
